...
public struct GestureComponent: Component, Codable {
    ...
    public var anchoredToHand = false
...
//            if anchoredToHand {
//                state.dragStartPosition = entity.position(relativeTo: entity.parent)
//            }
        }
   
        var translation3D = value.convert(value.gestureValue.translation3D, from: .local, to: .scene)
        
        if anchoredToHand {
            translation3D = value.convert(value.gestureValue.translation3D.rotated(by: .init(angle: .degrees(-90), axis: .y)), from: .local, to: .scene)
            
            translation3D.x = -translation3D.x
            translation3D.z = -translation3D.z
            print("anchored")
        }
